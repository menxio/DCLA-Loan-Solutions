...
export class LoansService {
  ...
  async reloan(loanId: string, dto: ReloanDto) {
    ...
    // Persist net cash released into today's collection for visibility in collections table
    try {
      ...
      if (borrower && borrower.center?.id) {
        // Try to find existing collection for today
        let collection = await this.collectionRepository.findOne({
          where: { memberId: borrower.id, centerId: borrower.center.id, collectionDate: today },
        });

        if (!collection) {
          collection = this.collectionRepository.create({
            memberId: borrower.id,
            centerId: borrower.center.id,
            collectionDate: today,
            amount: 0,
            paymentReceived: 0,
            netRelease: Number(netCashReleased) || 0,
            isAutoGenerated: true,
            notes: 'Net cash released from reloan',
            numberOfPayments: 0,
            advancePaymentAmount: 0,
            advancePaymentStatus: 'none' as any,
          });
        } else {
          collection.netRelease = Number(netCashReleased) || 0;
          if (!collection.notes) {
            collection.notes = 'Net cash released from reloan';
          }
        }
      ...
      // Silently ignore collection write errors to not block reloan
    }
